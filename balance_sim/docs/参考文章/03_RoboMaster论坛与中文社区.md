...
| 资料 | 链接 | 简介 | 参考价值 |
|---|---|---|---|
| **RoboMaster 平衡步兵机器人控制系统设计** | https://zhuanlan.zhihu.com/p/563048952 | 平衡步兵底盘建模为**轮腿倒立摆模型**：状态 $x=[\theta,\dot{\theta},x,\dot{x},\varphi,\dot{\varphi}]^{\mathsf{T}}$、控制 $u=[T,Tp]^{\mathsf{T}}$，MATLAB 符号法建非线性模型并线性化，验证可控可观性，LQR 求反馈阵 | **与本项目完全同构的建模路线**（串联腿/轮腿倒立摆 6 状态 + LQR），可直接对照状态选取、线性化方法与 LQR 设计 |
...
| **《早期人类如何驯服平衡车？LQR 控制与 Simulink 倒立摆仿真》** | https://zhuanlan.zhihu.com/p/380226006 | 平衡车 = 倒立摆建模（$x, v, \theta, \omega$ 四状态）、LQR 求解步骤（Q/R → Riccati → 最优控制律）、Simulink 仿真 | 自平衡车 LQR 建模→调参→仿真的完整入门路线 |
